...
void sauver_rasterfile(const char *nom, int largeur, int hauteur, unsigned char *p)
{
	FILE *fd;
...
/**
 * Kernel CUDA : chaque thread traite le pixel (i, j) de l'image.
 *
 * @param d_ima		L'image en mémoire GPU
 * @param width		Largeur de l'image en pixels
 * @param height	Hauteur de l'image en pixels
 * @param prof		Nombre maximum d'itérations par pixel
 * @param xmin		Coordonnée minimale sur l'axe x
 * @param ymin		Coordonnée minimale sur l'axe y
 * @param xinc		Pas d'incrément sur l'axe x
 * @param yinc		Pas d'incrément sur l'axe y
 */
__global__ void kernel_create_image(
	unsigned char *d_ima,
	int width,
	int height,
	int prof,
	...
	double yinc)
{
	...
	d_ima[i * width + j] = xy2color(x, y, prof);
}
...
int main(int argc, char *argv[])
{
	...
	ima_size = w * h * sizeof(unsigned char);
	ima = (unsigned char *)malloc(ima_size);
	...
	/* Alloue un tableau de ima_size octets sur le GPU afin d'y effectuer les
	 * calculs avec le kernel.
	 */
	cudaMalloc((void **)&d_ima, ima_size);

	/* Chaque bloc contient BLOCK_SIZE x BLOCK_SIZE threads (16x16 = 256 threads/bloc)
	 * Le nombre de blocs est calculé pour couvrir toute l'image, même si w ou h n'est
	 * pas un multiple de BLOCK_SIZE
	 */
	...
	kernel_create_image<<<nBlocs, blockSize>>>(d_ima, w, h, prof, xmin, ymin, xinc, yinc);

	/* le résultat du tableau GPU (d_ima) est transféré vers le tableau CPU (ima),
	 * puis libère la mémoire GPU allouée.
	 */
	cudaMemcpy(ima, d_ima, ima_size, cudaMemcpyDeviceToHost);
	cudaFree(d_ima);
	...
	sauver_rasterfile("mandel_cuda.ras", w, h, ima);

...
__device__ unsigned char xy2color(double a, double b, int prof, unsigned long *d_iter_count) {
...
__global__ void create_image(
        ...
        d_ima[i * width + j] = xy2color(x, y, prof, d_iter_count);
}
...
int main(int argc, char *argv[]) {
  ...
  iter_count = (unsigned long *) malloc(sizeof(unsigned long));
  ...
  cudaMalloc((void **) &d_iter_count, sizeof(unsigned long));
  dim3 blockSize(BLOCK_SIZE, BLOCK_SIZE);
  dim3 nBlocs(
        (w + blockSize.x - 1) / blockSize.x,
        (h + blockSize.y - 1) / blockSize.y
        // ceil(w / (float) BLOCK_SIZE),
        // ceil(h / (float) BLOCK_SIZE)
  );

  create_image<<<nBlocs, blockSize>>>(d_ima, w, h, prof, xmin, ymin, xinc, yinc, d_iter_count);

  // Ancienne version CPU
  //
  ...
  cudaMemcpy(iter_count, d_iter_count, sizeof(unsigned long), cudaMemcpyDeviceToHost);
  cudaFree(d_ima);
  ...
  fprintf( stderr, "Temps total de calcul : %g sec\n",
           fin - debut);
  fprintf(stdout, "nombre iteration total : %lu\n", iter_count);
  fprintf(stdout, "nombre d'iteration en moyenne : %lg\n", (double) (*iter_count) / (double) (w * h));
